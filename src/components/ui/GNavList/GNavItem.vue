<script setup lang="ts" generic="T extends NavValue = NavValue">
	...
	import { useNavGroupInject } from './groupContext';
	import {
		makeNavItemProps,
		type GNavItemSlotProps,
		type GNavItemSlots,
		type NavValue
	} from './types';

	const props = defineProps(makeNavItemProps());
	const slots = defineSlots<GNavItemSlots<T>>();
	const emit = defineEmits<{
		click: [event: MouseEvent];
	}>();

	const nav = useNavListInject<T>();
	const group = useNavGroupInject<T>();
	const RouterLink = useOptionalRouterLink();
	const selected = computed(() =>
		nav ? nav.isSelected(props.value as T | undefined) : false
	);
	const placement = computed(() => nav?.placement.value ?? 'left');
	const align = computed(() => nav?.align.value ?? 'start');
	const indicator = computed(() => nav?.indicator.value ?? 'line');
	...
	const slotProps = computed<GNavItemSlotProps<T>>(() => ({
...
<template>
	<li
		class="g-nav-item"
		:class="{
			'g-nav-item_selected': selected,
			'g-nav-item_disabled': props.disabled,
			[`g-nav-item_${placement}`]: true,
			[`g-nav-item_align-${align}`]: true,
			[`g-nav-item_indicator-${indicator}`]: true
		}"
		:style="itemStyles"
		role="none">
		<component
			:is="actionTag"
			v-ripple
			class="g-nav-item__action"
			:aria-current="selected ? 'page' : undefined"
			v-bind="actionAttrs"
			@click="onClick">
			<span
				v-if="slots.prepend || props.prependIcon"
				class="g-nav-item__prepend">
				<slot
					name="prepend"
					v-bind="slotProps">
					<g-icon
			...
			<span class="g-nav-item__label">
				<slot v-bind="slotProps">{{ props.label }}</slot>
			</span>

			<span
				aria-hidden="true"
				class="g-nav-item__indicator"></span>

			<span
				v-if="slots.append || props.appendIcon"
				class="g-nav-item__append">
				<slot
					name="append"
					v-bind="slotProps">
					<g-icon
...
<style scoped lang="scss">
	.g-nav-item {
		...
		&_indicator-dot &__indicator {
			...
			background: var(--g-nav-list-active-gradient);
